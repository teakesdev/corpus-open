...
- R2: `courtlistener-free` adapter behind a written grant. *(not started; separately gated — go required.)* **Consent-authoring constraint (chair, 2026-09-08, supersedes this seat's earlier CLI framing):** an agent-callable MCP/CLI consent command may only **stage a proposal** — an inert file no `kind:"human"` grant can become effective merely because an agent wrote one. Activation requires a step **outside the agent's write authority**: an interactive human confirmation run by the human, a separately-held broker key, or an OS-user-owned grant store. Agent-authored files (however labeled) stay inert until that out-of-band activation; `authorized_callers` in a proposal bind no one until a human activates it. Honesty rule carried from the trust model: an activation marker living in the same writable environment is forgeable and counts as *in* the agent's authority — the kit's promise is "its own call graph refuses + a cooperative agent cannot self-authorize," never "the system cannot egress." Caller-identity ergonomics (how an agent run declares `caller_id`) also land in R2.
