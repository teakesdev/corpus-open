...
- R2: `courtlistener-free` adapter behind a **human-activated** grant. **LANDED 2026-09-09** (`matter search`, `consent propose`/`activate`). Agent propose is inert; activate requires `/dev/tty` + `ACTIVATE`. Manual queries only; results carry CourtListener URL provenance and may be stored as `resolved`, **never** `source-checked`. Terms/API reviewed: free throttle, not legal advice, query_text only, no new paid tier. *(Corpus adapter still R3 / founder-gated.)*
