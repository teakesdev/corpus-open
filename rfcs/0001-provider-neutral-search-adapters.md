...
- R2: `courtlistener-free` adapter behind a written grant. *(not started; separate go)* **R2 scope must also add the grant-creation surface Deepseek flagged (2026-09-08): a CLI/MCP `consent` command that creates/modifies `.matter/consent.json` — human-issued `kind:"human"` grants naming `authorized_callers` explicitly — plus caller-identity ergonomics (how an agent run declares its caller_id), since R1 ships enforcement but no production path that authors grants.**
