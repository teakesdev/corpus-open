...
**Status:** PROPOSED rev4 — R1 CLOSED (chair, 2026-09-08); **R2 CLOSED** (`5193d92`, Deepseek APPROVE 2026-09-09, chair accepted); **R3 CLOSED within scope** (`f1905a1`, Deepseek APPROVE 2026-09-09, chair accepted — live Corpus MCP has no `law.verify_citation`, so no path to `source-checked`); kit publish still founder-gated; Voyage spend unapproved
...
Still founder-gated: publishing the kit remote. R3 Corpus adapter is **implemented and reviewed** (`f1905a1`) for *manual* search/lookup only; it is not a grant to auto-call, spend Voyage, or publish.

## 6. Rollout (small, separate commits)

- R1: **LANDED 2026-09-08, rev4 after adversarial review; CLOSED by chair 2026-09-08** (independent reproductions: Deepseek, grok — 35/35 at `1400e76`; record pinned at `05e74fb`). Closure accepts: local extraction + stage handling, cooperative consent enforcement, demonstrated Python-level isolation. Closure does NOT accept: benchmarked parser quality (Set A/B/C still owed), OS-level containment (never claimed). Evidence: legacy-store migration, CLI/MCP regression, offsets-vs-text-sha, **Python-level network denial inside the MCP child** (`tests/netblock_sitecustomize/sitecustomize.py` via PYTHONPATH) with **two negative controls**, AST import-graph scan, consent matrix (listed/unlisted caller, scope overflow, inert agent grants, expiry, escalation, budget). Packet count updated for the new vocabulary. Adversarial corrections folded: Deepseek (subprocess isolation; guard gap), chair (denial inside the child; issuer/callers/scope).
- R2: `courtlistener-free` adapter behind a **human-activated** grant. **LANDED 2026-09-09** (`5c3d423` then transport fix `5193d92`). **CLOSED** after independent re-verify (Deepseek: redirect denial reproduced — zero requests on destination, no `Authorization` forwarded; authenticated without token fails before egress; provenance `authenticated: true|false`). Agent propose is inert; activate requires `/dev/tty` + `ACTIVATE` (cooperative confirmation, not a compromised-agent trust anchor). Manual queries only; results carry CourtListener URL provenance and may be stored as `resolved`, **never** `source-checked`. Terms/API reviewed: free throttle, not legal advice, query_text only, no new paid tier.
- R3: `corpus-search` adapter. **LANDED 2026-09-09** (`0ef6440` discovery + `f1905a1` implementation). **CLOSED within scope** after independent review (Deepseek APPROVE; Flash verification). Live public MCP (`corpus-legal` v1.2.1) exposes `law.search` + `law.get_node` and **does not** expose `law.verify_citation` — `source-checked` is unreachable by construction. Protocol errors (`-32xxx` / `isError:true`) are distinct from legitimate empty results. `get_node` consent covers the outgoing node id. Auth mode explicit; redirects refused; keyed mode fails before egress without `CORPUS_API_KEY`. Residual (not a blocker): hit parser depends on the live tool's text shape — CI smoke against the real endpoint is hardening when R3 is used in anger, not a closure condition.
